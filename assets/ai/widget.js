...
if (!document.getElementById(ROOT_ID)) {
  ...
  .nkx-fab{
    ...
    background:var(--black,#030507); border:1px solid var(--line-lit,#2A353C);
    cursor:pointer; display:flex; align-items:center; justify-content:center;
    box-shadow:0 8px 28px rgba(0,0,0,.45);
    transition:transform .3s var(--ease-out,ease), border-color .3s;
  }
  .nkx-fab:hover{ transform:translateY(-2px); border-color:var(--accent,#8FB8C4); }
  .nkx-fab img{ width:28px; height:28px; border-radius:50%; object-fit:cover; }
  .nkx-fab-ring{
  ...
  .nkx-fab-dot{
    ...
    background:var(--accent,#8FB8C4); border:2px solid var(--black,#030507);
  }
  ...
  root.innerHTML = `
    ...
    <div class="nkx-panel" role="dialog" aria-label="NUKRAX AI chat">
      <div class="nkx-head">
        <div class="nkx-head-left">
          <span class="nkx-head-dot"></span>
          <div>
            <div class="nkx-head-title">NUKRAX AI</div>
    ...
    <button class="nkx-fab" data-fab type="button" aria-label="Open NUKRAX AI chat">
  ...
  function renderMsg(m) {
    ...
    if (m.role === 'user') {
      ...
      el.innerHTML = `<span class="nkx-msg-ai-label">NUKRAX AI</span><span class="nkx-msg-ai-text"></span>`;
  ...
  async function sendMessage() {
    ...
    // Nukrax Assistant answers locally — no network call, no daily limit, no cost.
    if (mode === 'nukrax') {
      const delay = 320 + Math.random() * 260;
      await new Promise(r => setTimeout(r, delay));
      typing.remove();
      const reply = getNukraxResponse(text);
    ...
    // General AI uses the free Workers AI binding via the Worker.
    try {
      ...
      typing.remove();
      convo = History.addMessage(convo.id, 'assistant', "I'm having trouble connecting right now. Please try again shortly, or reach out on Telegram (@CosmoLanex).");
      renderMessages();
    } finally {
      sendBtn.disabled = false;
    }
