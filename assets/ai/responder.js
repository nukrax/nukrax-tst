...
// It works by scoring the user's message against the structured
// NUKRAX_KNOWLEDGE data (FAQ, EAs, contact, pages) and returning the
// best match, or an honest fallback pointing to Telegram.
// ═══════════════════════════════════════════════
...
export function getNukraxResponse(rawMessage) {
  ...
  if (/^(hi|hello|hey|yo|sup|good (morning|afternoon|evening))\b/.test(lower) && message.length < 30) {
    return "Hey! I'm the NUKRAX Assistant. Ask me about the Expert Advisors, setup, risk management, or how to get in touch — happy to help.";
  }
  ...
  if (includesAny(lower, ['who are you', 'what are you', 'what can you do', 'what is this'])) {
    ...
    return `I'm the NUKRAX Assistant — I can answer questions about ${k.name}: ${k.description} Ask me about the Expert Advisors, setup, risk, or contact info.`;
  }
  ...
  // ── Honest fallback ──
  return "I don't have that on file. I can help with Expert Advisors, setup, risk management, brokers, or how to reach the team — try rephrasing, or reach out directly on Telegram (@CosmoLanex) for anything more specific.";
}
